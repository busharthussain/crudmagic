...
class ControllerCommand extends GeneratorCommand
{
     ...
     * @var string
     */
    protected $signature = 'magic:controller {name} {--blank}';
     ...
     * @var string
     */
    protected $description = 'Generate a resource controller with Cruds implementation';
    ...
    protected function replaceViewPathName($name)
    {
        $class = str_replace($this->getNamespace($name) . '\\', '', $name);
    ...
    protected function replaceModelName($name)
    {
        $class = str_replace($this->getNamespace($name) . '\\', '', $name);
    ...
    protected function buildClass($name)
    {
        ...
        $replace = array_merge($replace, [
            ...
            'DummyModel' => $this->replaceModelName($name),
        ]);
    ...
    {
        if ($this->option('blank')) {
            $this->comment('Building new controller');
        } else {
